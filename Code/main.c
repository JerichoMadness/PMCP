...
void getChainOrder(int **split, int *order,  int i, int j, int pos) {
	...
	if (i < j) {
		...
		getChainOrder(split,order, i, k,  pos-2);

		getChainOrder(split,order, k+1, j, pos-2);
...
int main() {
	...
	getChainOrder(split, order, 0, n-1, 2*(n-1));
