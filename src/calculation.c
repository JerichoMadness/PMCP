...
double calculateChainIterativeNEW(double **A, double **interRes, int *sizes, struct node *nd, int N)  {

    double timeB4, timeAfter, timeSum;
    timeSum = 0.;

    if((nd->cLeft) != NULL) {
        timeSum = timeSum + calculateChainIterativeNEW(A,interRes,sizes,nd->cLeft,N);   
    }

    if((nd->cRight) != NULL) {
        timeSum = timeSum + calculateChainIterativeNEW(A,interRes,sizes,nd->cRight,N);
    }
